...
#### 현재 배포 구성 (D40 기준) — 근거는 3.7·4.22

| 단계 | 채택 | 대체된 구성 |
|---|---|---|
| 간판 검출 | YOLO26x (AP@0.5 0.860) | — |
| 단어 검출 | 학습된 YOLO26x 단어 탐지기 @ conf 0.01 | CRAFT ∪ PaddleOCR-DB (D38) |
| 라인 구성 | y_tol 0.04 그룹핑 → union bbox + pad 0.04 한 장 | per-box 인식 (D20) |
| 인식 | **SVTRv2-B 단일** (GT 크롭 75.5%) | PaddleOCR PP-OCRv5 3-way 투표 (D40) |
| VLM 교정 | Gemma 4 31B / Qwen3-VL-32B-instruct — 모드 재측정 중 | Gemma 4 fixcand5 (옛 검출기 기준) |
| 의미 태깅 | Gemma 4 31B | — |

**측정 대기:** 간판→단어 연쇄 AP@0.5, EasyOCR·TrOCR-base 라인 스트립, VLM 2모델 × 3모드.
아래 이력표는 시간순 기록입니다. 현행 구성과 어긋나 보이면 위 표와 4.22 가 우선합니다.
...
| D40 | **배포 인식기 SVTRv2 확정 · 표 4개 체계 · 연쇄 AP** | YOLO26x 검출기 위에서 3-way 투표 이득이 +2.7 → **+0.5%p** 로 줄어(투표가 메우던 건 검출 잡음) 앙상블을 버리고 **SVTRv2-B 단일**을 배포로 확정 — GT 크롭 74.0 → **75.5%**, 인식 패스 3→1회. 단 **VLM 교정 후에는 83.8 vs 83.6% 로 동률**이라 최종 성능의 근거로는 못 씀(배포 근거는 같은 성능을 1/3 비용에). 간판→단어 **연쇄 AP@0.5**(`eval_text_chain.py`) 신설 — 간판 검출 실패·허위가 그대로 전달되는 실제 거동을 단독 AP 와 같은 단위로 측정. EasyOCR·TrOCR-base 라인 스트립 워커 추가, Surya 는 **학습 스크립트 부재(미세조정 불가) + 논문·Q1 선례 부재**로 제외하고 그 자리에 같은 하네스로 학습한 ABINet 을 넣기로 함 | 4.22 |
| D39 | **OCR 통제 비교 (앙상블 제외 · val/test 분리 · 신뢰구간 · YOLO26x 검출기 고정)** | 4.19 표는 배포 성능용이라 앙상블 행이 섞이고 점추정뿐이었음. 단일 모델만 두고 사진 단위 val/test 분리 + 크롭 페어 부트스트랩 95% CI. **미세조정 현대 인식기 3종(PaddleOCR·SVTRv2·PARSeq)은 구분 불가한 한 덩어리**(SVTRv2−PaddleOCR +3.5 [−0.7, +7.9]), CLOVA 는 −13.8 [−19.6, −8.2] 로 유의하게 뒤짐. 같은 비교를 옛 CRAFT∪DB 검출기에서 하면 SVTRv2 승(+5.2 유의)·CLOVA 무승부로 **순위가 뒤집힘** → 인식기 비교는 검출기를 명시해야 함. EasyOCR·TrOCR 는 라인 스트립 실행이 없어 제외 | 4.21 |
| D38 | **텍스트 검출기 교체 — 학습된 YOLO26x 단어 탐지기** | 배포 CRAFT∪DB 를 4.18 의 YOLO26x 단어 탐지기로 교체(검출 단계만, 나머지 동일). conf 0.01 최적. **GT 크롭 69.9 → 74.0%**(수원 +9.9%p), 교정 후 80.9 → **83.8%**. 연쇄는 54.4% 동률이나 언어권으로 나누면 **55.9%**, 태깅 연쇄 66.6 → **68.9%**. 수원 손실의 상당 부분이 인식이 아니라 검출이었음 | 4.20 |
...
> **대체됨 (D38).** CRAFT 는 더 이상 배포 검출기가 아닙니다 — 학습된 YOLO26x 단어 탐지기로 교체됐습니다(4.20).
> 이 절은 EasyOCR 인식기(CRNN) 학습 기록으로만 유효하고, 그 인식기는 4.22 의 비교표에 한 행으로 들어갑니다.
...
> **대체됨 (D40).** PP-OCRv5 는 더 이상 배포 인식기가 아닙니다 — SVTRv2-B 로 교체됐습니다(3.7·4.22).
> 이 절의 학습 기록은 그대로 유효하고, PP-OCRv5 는 비교표의 기준 행으로 남습니다.
...
### 3.7 배포 OCR 파이프라인 (`run_ocr_line.py`) — **현행 (D40)**

> **이 절이 배포 구성의 유일한 출처입니다.** 3.5·3.6·4.4·4.8·4.9·4.14 에는 이전 배포 구성이
> 남아 있지만 전부 대체된 내용이고, 각 절 머리에 무엇으로 대체됐는지 적어 두었습니다.
> 서로 어긋나 보이면 이 절이 맞습니다.

사진 1장 → 간판 크롭 → 텍스트 라인들 → 의미 태그.

```
GSV 사진
  │
  ├─⓪ 간판 검출: YOLO26x (imgsz 960, conf 0.25)                          ← 4.3·4.12
  │
  ├─① 단어 검출: 학습된 YOLO26x 단어 탐지기 @ conf 0.01                   ← 4.20 (D38)
  │      · 이전 구성(CRAFT ∪ PaddleOCR-DB)을 대체. GT 크롭 69.9 → 74.0%
  │      · conf 는 0.35~0.001 을 훑어 0.01 에서 최대, 양쪽으로 꺾임
  │
  ├─② 라인 그룹핑: y-중심 클러스터링(y_tol 0.04·이미지높이) → x 정렬       ← 4.7
  │
  ├─③ 라인 병합 크롭: 라인의 모든 박스를 union bbox + pad 0.04 로 한 장    ← 4.7
  │      · 핵심: 박스별로 자르지 않고 라인째 인식 (per-box 대비 +16.5%p)
  │
  ├─④ 인식: SVTRv2-B (OpenOCR, 격리 subprocess), 전 지역 단일 모델        ← 4.22 (D40)
  │      · 이전 구성(PaddleOCR PP-OCRv5 3-way 라인 다수결)을 대체
  │      · GT 크롭 74.0 → 75.5%, 인식 패스 3회 → 1회
  │      · 단, 통제 비교의 held-out 절반에서 PaddleOCR 대비 +3.5%p
  │        [-0.7, +7.9] 로 **유의하지 않음** — 점추정 1등이라 고른 것이며
  │        "SVTRv2 가 PaddleOCR 보다 낫다"고 단정하면 안 됩니다
  │
  ├─⑤ 조립: 라인 텍스트를 `\n`로 이어 `ocr_{region}_{run}_{engine}.csv`
  │
  └─⑥ VLM 교정: Gemma 4 31B / Qwen3-VL-32B-instruct (Ollama 4-bit)        ← 4.22
         · 모드 fixtext / fix / fixcand 를 이 검출기·이 인식기 위에서 재측정 중
         · 이전 공식 채택(Gemma 4 fixcand5, 4.14)은 CRAFT∪DB 스트립에서 잰 값이라
           그대로 쓰지 않습니다
  │
  └─⑦ 의미 태깅: Gemma 4 31B                                              ← 4.13
```

> ⚙️ **PaddleOCR 격리 (여전히 유효):** paddle(cu118)과 torch(cu118)가 **동일 이름의 cudnn_*_8.dll
> (다른 ABI)** 를 ship 하여 한 프로세스에 공존하면 `WinError 127` 이 납니다. PaddleOCR 를 쓰는
> 경로는 별도 subprocess 로 격리합니다 — `paddle_rec_worker.py` / `paddle_det_worker.py`.
> SVTRv2·PARSeq·EasyOCR·TrOCR·Tesseract·Surya·CLOVA 도 같은 IO 계약의 워커로 분리돼 있어
> `--worker` 하나만 바꾸면 인식기가 교체됩니다.

```bash
# 현행 배포 (YOLO26x 단어 검출 + SVTRv2-B 인식)
...
> **대체됨 (D38·D40).** 아래 수치는 CRAFT∪DB 검출 + PaddleOCR 3-way 투표 시절의 배포 성능입니다.
> 현행 배포 수치는 4.22 에 있습니다. 이 절은 그때의 기록으로만 읽어 주세요.
...
> **부분 대체 (D40).** v5_lines 학습 자체는 유효하지만, 배포 인식기는 SVTRv2-B 입니다(3.7).
...
> **대체됨 (D38).** CRAFT ∪ PaddleOCR-DB 는 학습된 YOLO26x 단어 탐지기로 교체됐습니다(4.20).
> 아래는 교체 전 기록이며, 현행 표에는 이 구성이 들어가지 않습니다.
...
> **재측정 중 (D40).** 아래 fix/fixcand/fixcand5 비교는 CRAFT∪DB 스트립에서 잰 값입니다.
> 검출기와 인식기가 모두 바뀌었으므로 같은 비교를 현행 구성에서 다시 돌리고 있습니다(4.22).
...
즉 **같은 모델 계열을 두 탐지 과제에 각각 학습했을 때**의 수치.

> ⚠️ **연쇄(cascade)가 아닙니다.** 텍스트 박스 AP 는 **원본 사진 전체**(AI Hub 1920×1400급)에 단어 탐지기를 직접 돌려 잰 값이고,
> 1단계가 예측한 간판 크롭 안에서 잰 것이 아닙니다. 간판 AP(GSV 298장)와 텍스트 AP(AI Hub test 2,706장)는 **데이터셋도 다릅니다**.
> 두 수치를 곱하거나 이어 붙이면 안 되고, 한 행은 "이 아키텍처를 두 과제에 각각 학습한 결과"로 읽어야 합니다.
> 실제 연쇄 성능은 4.16 이고, 거기서 쓰는 텍스트 검출기는 이 4모델이 아니라 배포 구성인 CRAFT ∪ PaddleOCR-DB 입니다. 4.3.1 의 텍스트 탐지는 Signboard
...
### 4.20 텍스트 검출기 교체 — 학습된 YOLO26x 단어 탐지기 (D38)

**질문:** 배포 검출기(CRAFT ∪ PaddleOCR-DB)를 4.18에서 학습한 **YOLO26x 단어 탐지기**로 갈아끼우면 파이프라인이 좋아지는가.

**설계.** `run_ocr_line.py --text-detector {union,craft,yolo,yolo_union}` 로 **검출 단계만** 교체합니다. 라인 병합(y_tol 0.04),
스트립 패딩(0.04), 인식기(PP-OCRv5 v5_lines·한국어권 3-way vote·브루클린 en), 전처리, 채점은 전부 동일합니다.
검출기 워커는 `str_baselines/yolo_text_det_worker.py`(paddle_det_worker 와 같은 IO 계약).

**conf 훑기 (GT 크롭).** 0.35 → 0.001 로 훑으면 line exact 가 **0.01 에서 최대(74.0%)** 이고 양쪽으로 꺾입니다.
요청받은 0.15/0.25/0.35 구간만 봤다면 69.4% 로 "배포보다 나쁘다"는 반대 결론이 나왔을 값입니다.

| conf | 0.35 | 0.25 | 0.15 | 0.10 | 0.05 | 0.02 | **0.01** | 0.005 | 0.001 |
|---|---|---|---|---|---|---|---|---|---|
| 단어 박스 | 991 | 1,183 | 1,441 | 1,634 | 1,945 | 2,450 | **2,992** | 3,757 | 6,730 |
| line exact | 64.4 | 67.6 | 69.4 | 71.1 | 72.5 | 73.1 | **74.0** | 73.1 | 73.5 |
| CER | .235 | .195 | .169 | .165 | .151 | .154 | **.148** | .163 | .183 |

**GT 크롭 (411 크롭 / 592 라인) — 검출기만 교체:**

| 검출기 구성 | 강남 | 브루클린 | 수원 | 전체 | CER | WER | 미회수 라인 |
|---|---|---|---|---|---|---|---|
| CRAFT 단독 | 69.8 | 74.9 | 56.4 | 67.4% | 0.193 | 0.545 | 76 |
| CRAFT ∪ DB (현행 배포) | 73.1 | 76.9 | 58.6 | 69.9% | 0.163 | 0.540 | 53 |
| **YOLO26x @0.01** | **75.0** | 77.9 | **68.5** | **74.0%** | **0.148** | **0.478** | **35** |
| YOLO26x ∪ CRAFT | 75.0 | 78.4 | 68.5 | 74.2% | 0.150 | 0.503 | 35 |
| 언어권 혼합(한국어권 YOLO / 영어권 CRAFT∪DB) | 75.0 | 76.9 | 68.5 | 73.7% | 0.149 | 0.505 | 37 |

**GT 크롭 + VLM 교정 (Gemma 4 31B fixcand5, 후보 5종은 새 스트립 기준 재생성):**

| 검출기 구성 | 강남 | 브루클린 | 수원 | 전체 | CER |
|---|---|---|---|---|---|
| CRAFT ∪ DB (현행) | 82.5 | 88.9 | 70.2 | 80.9% | 0.122 |
| **YOLO26x @0.01** | **83.5** | 88.4 | **79.0** | **83.8%** | 0.120 |
| 언어권 혼합 | 83.5 | 88.9 | 79.0 | 84.0% | **0.116** |

**연쇄 (탐지 크롭 502개) — 여기서 그림이 뒤집힙니다:**

| 구성 | 강남 | 브루클린 | 수원 | 전체 | TP 위 정확도 |
|---|---|---|---|---|---|
| CRAFT ∪ DB, OCR만 | 50.0 | 46.2 | 32.0 | 43.2% | 52.1% |
| YOLO26x, OCR만 | 50.0 | 43.7 | 44.8 | 46.3% | 55.8% |
| CRAFT ∪ DB + VLM (기존 배포) | 59.9 | 58.3 | 43.6 | 54.4% | 65.6% |
| YOLO26x + VLM | 56.6 | 53.8 | 52.5 | 54.4% | 65.6% |
| **언어권 규칙 + VLM** (사전 규칙) | 56.6 | 58.3 | 52.5 | **55.9%** | **67.4%** |
| 지역별 최선 + VLM (사후 선택 = 상한) | 59.9 | 58.3 | 52.5 | 57.1% | 68.8% |

**연쇄 태깅:** CRAFT∪DB 66.6% / TP위 80.2% → **YOLO26x 68.9% / 82.9%**.

**해석:**
1. **GT 크롭에서는 YOLO 가 전 지역 우세**입니다(+4.1%p, 교정 후 +2.9%p). 미회수 라인 53→35, 빈 크롭 3→0.
   **수원 +9.9%p** 가 핵심입니다. 지금까지 수원을 "읽기가 어려운 지역"으로 서술해 왔는데, 손실의 상당 부분이
   인식이 아니라 **검출**이었습니다. 양각·저대비 간판에서 CRAFT 가 글자를 놓치고 있었습니다.
2. **연쇄에서는 무승부**입니다(54.4% 동률). 수원 +8.9%p 를 강남 −3.3·브루클린 −4.5 가 정확히 상쇄합니다.
   탐지 크롭은 GT 크롭보다 박스가 헐거워 conf 0.01 의 여유분이 허위 박스로 바뀌고, 브루클린은 이 탐지기의
   학습 분포(한국어 AI Hub 간판) 밖입니다. **GT 크롭 성능만 보고 배포를 결정하면 안 된다**는 사례입니다.
3. **언어권으로 검출기를 나누면 55.9%** 로 둘 다를 넘습니다(+1.5%p). 사후에 지역별 최선을 고른 57.1% 는
   test 를 보고 고른 값이라 상한 참고용이고, 언어권 규칙은 "탐지기가 한국어 간판으로 학습됐다"는
   **사전 근거**가 있어 배포 가능한 구성입니다.
4. **VLM 은 약한 검출기를 메우지 못합니다.** 검출기 이득이 교정 후에도 그대로 남습니다(+2.9%p).
   검출기가 애초에 잘라내지 않은 글자는 교정할 대상 자체가 없습니다.
5. **정밀도 대 recall 트레이드오프를 표에 같이 실어야 합니다.** conf 를 낮추면 line recall 86.1→94.1% 로 오르지만
   WER 은 0.449→0.478 로 나빠집니다. 허위 라인이 곧 허위 POI 이므로 지도 구축에서는 비용입니다.

**주의.** 4.18 의 0.822 는 **AI Hub 사진 전체**에서 잰 박스 AP 라 GSV 라인 exact 로 환산할 수 없습니다.
위 표는 전부 실제로 파이프라인을 돌려 나온 값입니다.

```bash
bash scripts/run_text_detector_ab.sh sweep     # conf 훑기
bash scripts/run_yolodet_hybrid.sh cand        # 후보 5종 재생성 (새 스트립)
bash scripts/run_yolodet_hybrid.sh vlm         # Gemma 4 fixcand5
bash scripts/run_yolodet_hybrid.sh cascade     # 연쇄 + 태깅
```

**함정 기록(D38).** ① 요청 구간(0.15~0.35)의 경계값이 최적이 아니었습니다 — 단조 추세면 구간 밖을 확인해야 합니다.
② OpenOCR·PARSeq 워커는 자기 저장소로 chdir 하므로 모델 경로는 **절대경로**여야 하고 PARSeq 는 `--ckpt` 가 필수입니다.
③ 새 구성을 `paddle` 태그로 저장하면 `eval_ocr_v2` 의 "엔진별 최신 run" 규칙과 충돌해 배포 행이 오염됩니다(D37 ④와 같은 사고)
→ `yolo01`·`yolodet` 으로 분리하고 `exp_vlm_ocr --deploy-engine`, `eval_e2e_cascade --gt-engine` 을 추가했습니다.

---

### 4.21 OCR 통제 비교 — 같은 조건에서 무엇이 무엇보다 나은가 (D39)

4.19 표는 배포 성능을 보이는 데 맞춰져 있어 **비교 실험으로는 두 군데가 어긋납니다**.
① 비교 대상에 **앙상블(3-way vote)** 행이 섞여 있습니다 — 모델이 아니라 시스템이라 같은 조건이 아닙니다.
② 점추정만 있어 "A가 B보다 낫다"를 뒷받침할 근거가 없습니다. 여기서는 그 둘을 고칩니다.

**프로토콜.** 고정: 검출기(**학습된 YOLO26x 단어 탐지기 @conf 0.01** — 4.20 에서 배포 검출기를 이긴 구성) ·
라인 병합(y_tol 0.04) · 패딩(0.04) · 전처리 · 채점 규칙. 바꾼 것은 **인식기 하나**.
미세조정군은 전부 같은 75,612 크롭 · 같은 signboard_v3 분할로 학습했습니다.
GSV 298장을 **사진 단위로 반씩** 갈라(seed 20260922) test 절반에서만 차이를 보고합니다(같은 간판이 양쪽에 걸리지 않음).
신뢰구간은 **크롭 단위 페어 부트스트랩** 95%(2,000회).
EasyOCR·TrOCR 는 **라인 스트립으로 돌린 적이 없어**(옛 per-box 파이프라인 전용 · 워커 없음) 이 비교에 넣을 수 없습니다.

| 인식기 | 학습 조건 | val exact | test exact | test CER |
|---|---|---|---|---|
| Tesseract 5.5 | 제로샷 | 31.7 | 32.2% | 0.420 |
| Surya 0.14 | 제로샷 | 41.3 | 43.9% | 0.308 |
| CLOVA OCR General | 제로샷(상용 API) | 52.8 | 58.8% | 0.213 |
| Tesseract 5.5 | 미세조정 | 34.7 | 33.6% | 0.404 |
| PARSeq (ViT-S) | 미세조정 | 68.3 | 69.2% | 0.143 |
| PaddleOCR (PP-OCRv5 rec) | 미세조정 | 74.6 | 72.7% | 0.115 |
| **SVTRv2-B** | 미세조정 | 75.2 | **76.1%** | **0.103** |

**차이와 신뢰구간 (test 절반, %p):**

| 인식기 | vs PaddleOCR | 95% CI | vs SVTRv2-B | 95% CI |
|---|---|---|---|---|
| **SVTRv2-B** | +3.5 | [−0.7, +7.9] · 미판정 | — | — |
| PaddleOCR (PP-OCRv5 rec) | — | — | −3.5 | [−7.9, +0.7] · 미판정 |
| PARSeq | −3.5 | [−8.4, +1.0] · 미판정 | −6.9 | [−10.9, −3.2] · 유의 |
| CLOVA OCR General | −13.8 | [−19.6, −8.2] · 유의 | −17.3 | [−23.3, −11.5] · 유의 |
| Surya | −28.7 | [−34.4, −23.1] | −32.2 | [−38.4, −26.4] |
| Tesseract 미세조정 | −39.1 | [−45.1, −33.2] | −42.6 | [−48.8, −36.4] |
| Tesseract 제로샷 | −40.5 | [−45.9, −34.7] | −43.9 | [−50.0, −37.8] |

**읽는 법과 살아남는 주장.** 구간이 0을 지나지 않으면 이 데이터가 뒷받침하는 차이이고, 0을 지나면 이 표본
크기(test 절반 296 라인)에서 **판정 불가**입니다. 판정 불가를 "이겼다"로 쓰면 안 됩니다.

1. **미세조정 현대 인식기 세 개(PaddleOCR · SVTRv2-B · PARSeq)는 한 덩어리입니다.** SVTRv2 −PaddleOCR
   +3.5 [−0.7, +7.9], PARSeq −PaddleOCR −3.5 [−8.4, +1.0] 모두 0을 지납니다. 유일하게 갈리는 쌍은
   SVTRv2 > PARSeq (−6.9 [−10.9, −3.2]) 하나뿐입니다.
2. **상용 API 는 이 검출기에서 미세조정군에 분명히 집니다** (−13.8 [−19.6, −8.2]).
3. 문서 OCR 계열(Tesseract·Surya)은 29~41%p 차이에 구간이 0 근처에도 오지 않습니다. 같은 데이터로
   미세조정해도 Tesseract 는 1.4%p 밖에 못 움직이므로, 이 격차는 데이터가 아니라 **구조**의 문제입니다.

**검출기를 바꾸면 인식기 순위가 뒤집힙니다 — 이게 이번 절의 실제 발견입니다.**
같은 비교를 옛 CRAFT∪DB 검출기에서 돌리면 **SVTRv2 > PaddleOCR +5.2%p [+1.1, +9.5] (유의)** 였고
CLOVA 는 PaddleOCR 과 **구분 불가**(−4.2, 0을 지남)였습니다. 검출기를 좋은 쪽으로 바꾸자 둘 다 뒤집힙니다.
① 인식기 격차로 보이던 것의 일부는 **검출 손실**이었고(강한 인식기가 그 손실을 더 잘 흡수했을 뿐),
② 깨끗해진 스트립의 이득은 미세조정군이 제로샷 API 보다 크게 가져가므로 CLOVA 의 열세가 −4.2 → −13.8 로 벌어집니다.
따라서 **인식기 비교는 어느 검출기 위에서 쟀는지를 반드시 밝혀야** 하고, 검출기가 다른 논문의 순위를 그대로 인용하면 안 됩니다.

**한계.** 모든 행이 공유하는 기하 파라미터(pad 0.04, y_tol 0.04)와 검출기 임계값(0.01, val 절반에서 선택)은
과거에 같은 GSV 데이터에서 정해졌습니다. 행 간 비교는 조건이 같아 공정하지만 **절대 수치는 모든 행에서 낙관적**입니다.
부트스트랩용으로 다시 센 크롭별 exact 는 공식 채점기와 최대 0.3%p 차이가 납니다(공식 채점기는 허위 라인도 계산에 넣음).
본문의 대표 수치는 공식 채점기, 구간은 재계산값입니다.

```bash
.venv/Scripts/python.exe str_baselines/eval_ocr_controlled.py    # 표·신뢰구간 재생성
```

---

### 4.22 배포 인식기 SVTRv2 확정 · 표 4개 체계 · 연쇄 AP (D40)

**무엇을 왜 바꿨나.** 4.20(D38)에서 검출기를 YOLO26x 로 바꾸고 나니, 그 위에서 3-way 투표가 벌어주는
이득이 **+2.7%p → +0.5%p** 로 줄었습니다. 투표가 고치고 있던 것은 인식 오류가 아니라 **검출이 지저분해서
생긴 흔들림**이었다는 뜻입니다. 투표는 스트립 하나당 인식 패스를 3회 쓰므로, 이득이 사라지면 남길 이유도
없습니다. 동시에 통제 비교(4.21)에서 SVTRv2-B 가 점추정 1등이었으므로 **배포를 SVTRv2-B 단일 모델로
확정**했습니다. 결과적으로 앙상블이 파이프라인에서 사라져, 비교표의 모든 행과 배포 구성이 **같은 종류의
대상(단일 모델)** 이 됩니다 — 4.21 에서 지적했던 "배포만 시스템이라 비교가 성립하지 않는다"는 문제가
구조적으로 해소됩니다.

| GT 크롭 411개 / 592 라인 | 강남 | 브루클린 | 수원 | 전체 | CER |
|---|---|---|---|---|---|
| CRAFT∪DB + PaddleOCR 3-way 투표 (옛 배포) | 73.1 | 76.9 | 58.6 | 69.9% | 0.163 |
| YOLO26x + PaddleOCR 3-way 투표 | 75.0 | 77.9 | 68.5 | 74.0% | 0.148 |
| YOLO26x + PaddleOCR 단일 | 73.6 | 77.9 | 68.5 | 73.5% | 0.149 |
| **YOLO26x + SVTRv2-B 단일 (현행 배포)** | 74.5 | 78.9 | **72.9** | **75.5%** | **0.147** |
| YOLO26x + PARSeq 단일 | 70.8 | 71.9 | 62.4 | 68.6% | 0.197 |

**단, 이 이득은 VLM 교정을 거치면 사라집니다.** 같은 fixcand5(Gemma 4 31B) 교정을 붙이면
PaddleOCR 투표 83.8% / SVTRv2 83.6% 로 **−0.2%p**, 사실상 동률입니다. 교정 전 +1.5%p 가 교정 후 0 이
된다는 것은 **VLM 이 두 인식기가 틀리는 지점을 비슷하게 메운다**는 뜻이고, 논문에 "더 좋은 인식기를 썼다"를
최종 성능의 근거로 내세우면 안 된다는 뜻이기도 합니다. 배포 선택의 근거는 성능이 아니라 **같은 성능을 1/3
비용으로 낸다**는 쪽이 정확합니다.

#### 표 체계 — 파이프라인 순서대로 4개

산출물은 `Desktop/GSV_pipeline_tables.docx` 이며 표는 정확히 4개입니다(소분할 없음).

| 표 | 내용 | 단위 |
|---|---|---|
| Table 1 | 간판 검출, 모델 × 지역 | AP@0.5 |
| Table 2 | 간판 검출 · 단어 박스 검출 · **둘의 연쇄**, 모델별 | AP@0.5 (세 열 같은 단위) |
| Table 3 | 인식기 단일 비교, 모델 × 지역 (+ 각 엔진의 정체와 채택 이유) | line exact / CER |
| Table 4 | SVTRv2 + VLM, 모델 × 모드 × 지역 | line exact / CER |

#### Table 2 — 연쇄를 AP@0.5 로 재는 법, 그리고 왜 AI Hub 에서 재는가

`detection/eval_text_chain.py` (신규). 지금까지 단어 검출기는 **원본 사진을 통째로 넣어** AP 를 쟀는데,
배포는 그렇게 돌지 않습니다. 실제로는 간판 검출기가 먼저 자르고 단어 검출기는 그 크롭만 봅니다.

```
사진 → 간판 검출(YOLO26x, conf 0.25, imgsz 960) → 크롭들
     → 크롭 안에서 단어 검출 → 박스를 원본 좌표로 되돌림 → NMS
     → 단어 GT 와 AP@0.5
```

**간판 검출기의 출력 자체는 채점하지 않습니다**(중간 단계). 그래서 간판 GT 가 필요 없고, 단어 GT 만 있으면
됩니다 — AI Hub hold-out(2,706장 / 78,110 박스)이 그 조건을 만족합니다. GSV 는 간판 박스는 있어도
**단어 박스 GT 가 없어** 같은 측정이 불가능하고, 지역별 AP 도 같은 이유로 존재하지 않습니다(추정치를 넣지
않고 빈칸으로 둡니다).

> **1단계 실패가 그대로 넘어가는지 — 넘어갑니다.** 간판 검출이 아무것도 못 찾은 사진은 크롭이 0개라
> 단어 예측도 0개가 되고, 그 사진의 GT 단어 박스는 전부 미검출로 AP 에 계산됩니다. 반대로 허위 간판
> 박스는 허위 크롭을 만들고 그 안의 단어 박스가 오검출로 들어갑니다. 즉 **회수율 손실과 정밀도 손실이
> 둘 다 연쇄로 전달**되며, 이것이 배포의 실제 거동입니다. 스크립트는 크롭이 하나도 안 나온 사진 수를
> 따로 출력해 이 손실의 크기를 드러냅니다.

#### Table 3 — 비교 대상의 근거

세 갈래로 나뉘며, 서로 다른 질문에 답하므로 하나의 순위로 읽으면 안 됩니다.

| 엔진 | 정체 | 넣은 이유 | 체급 |
|---|---|---|---|
| Tesseract 5.5 (제로샷/미세조정) | LSTM 라인 인식기, 오픈소스 문서 OCR 표준 | 응용 OCR 논문의 기본 기준선. 같은 데이터로 미세조정까지 되는 유일한 비-STR 엔진이라 "데이터 부족"과 "구조 문제"를 가릅니다 | 아래 |
| EasyOCR (CRNN, 미세조정) | CRNN 인식기 (JaidedAI) | 문헌에서 PaddleOCR 와 가장 자주 함께 벤치마크되는 상대 | 아래 |
| TrOCR-base (미세조정) | Transformer encoder-decoder OCR (Microsoft) | 같은 이유. small(62M) 대신 **base(334M)** 로 학습해 체급을 올렸습니다 | 비슷 |
| PARSeq (ViT-S, 미세조정) | Permuted AR STR, ECCV 2022 | 파라미터 수가 배포 인식기와 가장 가까운 **체급 일치** 비교 | 일치 |
| PaddleOCR PP-OCRv5 rec (미세조정) | 이전 배포 인식기 | 교체 판단의 기준 행 | 기준 |
| **SVTRv2-B (미세조정)** | Single visual model + CTC, OpenOCR, ICCV 2025 | **현행 배포** | 위 |
| CLOVA OCR General (제로샷) | NAVER 상용 API | 요청에 따른 상용 기준점. 미세조정이 불가능하므로 "유료 범용 API 가 미세조정 모델을 대체할 수 있나"에만 답합니다 | 비공개 |

**Surya 는 표에서 제외하고, 그 자리를 미세조정 가능한 모델로 채웁니다.** 제외 사유는 셋입니다.
① **미세조정이 불가능합니다.** 저장소(`datalab-to/surya`, 최신 태그 v0.22.1)의 `surya/scripts/` 에는
`detect_layout.py` · `detect_text.py` · `ocr_text.py` · `table_recognition.py` 등만 있고 **학습 스크립트가
없습니다**(GitHub API 로 디렉터리 목록을 직접 확인). README 도 "미세조정은 hi@datalab.to 로 문의"라며
자사 관리형 학습 서비스로 안내합니다. 설치된 `.venv_surya` 역시 surya-ocr 0.14.6 + **CPU 전용 torch** 라
학습 자체가 안 됩니다. ② 그래서 표에 넣으면 **제로샷 한 행만 미세조정군과 섞이는 불공정 비교**가 됩니다 —
낮은 점수가 구조 탓인지 학습을 못 한 탓인지 구분할 수 없습니다. ③ **논문이 없습니다.** GitHub 도구이고,
JCR Q1 저널 사용 선례를 찾지 못했습니다(사용 사례는 arXiv 프리프린트와 RANLP 2025 학회 논문뿐이며 모두
문서 OCR 맥락, 장면 텍스트 사례는 없음).

> **기록:** 이 문단은 한 번 틀렸다가 고쳐졌습니다. 검색 요약만 보고 "`finetune_ocr.py` 가 있으므로
> 미세조정이 가능하다"고 적었는데, 저장소를 직접 열어 보니 그런 파일이 없습니다. 근거는 검색 요약이 아니라
> 원본에서 확인해야 합니다.

**대신 같은 학습 조건의 최신 STR 모델을 추가합니다.** 제로샷 행을 빼고 그 자리를 채우는 쪽이 비교가
깨끗합니다. OpenOCR 저장소에는 SVTRv2 와 **같은 학습 하네스**로 돌릴 수 있는 구현이 들어 있어
(`configs/rec/` — abinet, maerec, cppd, lister, igtr …), 같은 75,612 크롭·같은 분할·같은 스케줄로
학습하면 조건이 완전히 동일해집니다. 1순위는 **ABINet**(CVPR 2021, 언어모델 결합 STR 의 표준 기준선),
2순위는 **MAERec**(ICCV 2023). 둘 다 앞서 확인한 *Sensors* 2025, 25(24), 7684 의 비교표에 행으로
들어가 있어 선례도 있습니다. 학습 비용은 같은 하네스 기준 SVTRv2 3h41m / PARSeq 2h15m 이었으므로
모델당 3~4시간 선입니다.

CLOVA OCR General 만 제로샷으로 남습니다. 상용 API 라 가중치를 만질 수 없다는 것이 **바꿀 수 없는 제약**
이고, 그 행이 답하는 질문("유료 범용 API 가 미세조정 모델을 대체할 수 있나") 자체가 제로샷을 전제로 하기
때문입니다. 표에 그렇게 명시합니다.

반면 PARSeq(ECCV 2022)·SVTRv2(ICCV 2025)는 최상위 학회 논문이고, 둘을 **한 논문의 비교표에서 동시에**
기준선으로 쓴 저널 사례가 있습니다 — Mu et al., *Sensors* 2025, 25(24), 7684 (2025-12-18). 다만 Sensors 의
JCR 분면은 분야에 따라 갈립니다(Instruments & Instrumentation Q1 / Engineering Electrical Q2).

#### Table 4 — VLM 교정, 모델 × 모드

배포 인식기(SVTRv2-B)를 기준선으로 고정하고, **모델 2종 × 모드 3종** 을 같은 스트립에서 돌립니다.

| 모드 | VLM 이 보는 것 | 답하는 질문 |
|---|---|---|
| fixtext | OCR 텍스트만 (**이미지 없음**) | 교정 이득 중 얼마가 시각 정보가 아니라 **언어 사전**에서 오는가 |
| fix | 간판 이미지 + OCR 텍스트 | 이미지를 주면 얼마가 더 붙는가 |
| fixcand | 간판 이미지 + 후보 3종(y5/y4/ypre) | 다른 인식기의 후보를 함께 보여주면 얼마가 더 붙는가 |

모델은 **Gemma 4 31B** 와 **Qwen3-VL-32B-instruct**(둘 다 Ollama 4-bit, GPU/RAM 분할). 4.14 의 이전
비교(CRAFT∪DB 스트립)는 검출기·인식기가 모두 바뀌었으므로 **가져오지 않고 다시 돌립니다**.

#### 새로 추가된 코드

| 파일 | 역할 |
|---|---|
| `detection/eval_text_chain.py` | 간판→단어 연쇄 AP@0.5. 채점기·해상도는 `eval_text_holdout.py` 와 동일하게 재사용해 단독 AP 와 같은 잣대 |
| `str_baselines/easyocr_rec_worker.py` | EasyOCR 인식기만 호출(검출 없음). `signboard_v3_custom` = 다른 미세조정 행과 같은 학습 데이터 |
| `str_baselines/trocr_rec_worker.py` | TrOCR 라인 인식. 기본은 `signboard_v3/trocr_model`, `--model` 로 base 체크포인트 교체 |
| `scripts/run_svtr_deploy.sh` | 배포 인식기를 SVTRv2 로 바꾼 전 구간 재측정 |
| `scripts/run_svtr_pipeline.sh` | 연쇄 AP → EasyOCR → TrOCR-base 학습·추론 → VLM 2×3 매트릭스 |

> **TrOCR-base 학습 주의.** `--out-dir` 아래에 모델이 저장되므로 small 모델을 덮어쓰지 않도록
> `artifacts/ocr_training/signboard_v3_base/` 를 따로 두고, 크롭 디렉터리는 **정션으로 연결**해
> 62,464장을 복사하지 않습니다. 분할·토크나이저(바이트레벨 BPE)는 3.4 의 signboard_v3 프로토콜 그대로라
...
|---|---|
| SVTRv2 GT 크롭 (75.5%) | ✅ |
| SVTRv2 + fixcand5 (83.6%) | ✅ |
| SVTRv2 연쇄(탐지 크롭) + 태깅 | ⏳ 실행 중 |
| 연쇄 AP@0.5 (4 아키텍처) | ⏳ 대기 |
| EasyOCR 라인 스트립 | ⏳ 대기 |
| TrOCR-base 학습 + 라인 스트립 | ⏳ 대기 |
| VLM 2모델 × 3모드 | ⏳ 대기 |

```bash
bash scripts/run_svtr_deploy.sh all      # 배포 인식기 SVTRv2 전 구간 재측정
bash scripts/run_svtr_pipeline.sh all    # 연쇄 AP + EasyOCR/TrOCR-base + VLM 매트릭스
...
# 8) OCR 추론 — 현행 배포(D40): YOLO26x 단어 검출 @0.01 + 라인 병합 + SVTRv2-B
.venv/Scripts/python.exe pipeline/run_ocr_line.py --run N \
    --text-detector yolo --yolo-det-conf 0.01 \
    --worker str_baselines/openocr_rec_worker.py \
    --worker-args "--config external/OpenOCR/configs/rec/svtrv2/svtrv2_rctc_signboard.yml \
                   --weights artifacts/str_baselines/svtrv2/best.pth" \
    --engine-tag cysvtrv2
# 인식기 교체는 --worker 만 (paddle_rec / parseq_rec / easyocr_rec / trocr_rec / tesseract_rec / surya_rec / clova_rec)
# 검출기 교체는 --text-detector {yolo,union,craft,yolo_union}  ← union·craft 는 교체 전 baseline 재현용

# 9) 평가
python detection/eval_map_only.py                              # 간판 탐지 mAP
.venv/Scripts/python.exe detection/eval_text_holdout.py        # 단어 박스 AP@0.5 (검출기 단독)
.venv/Scripts/python.exe detection/eval_text_chain.py          # 단어 박스 AP@0.5 (간판→단어 연쇄)
.venv/Scripts/python.exe pipeline/eval_ocr_v2.py --mask-phone --en-only-regions brooklyn   # OCR
.venv/Scripts/python.exe str_baselines/eval_ocr_controlled.py  # 인식기 통제 비교 + 신뢰구간
.venv/Scripts/python.exe pipeline/eval_e2e_cascade.py --ocr-run N --engine vlmfix          # 연쇄
```
