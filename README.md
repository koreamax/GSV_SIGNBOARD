...
**Surya 0.14 는 표에서 제외합니다.** 이유는 셋입니다. ① **논문이 없습니다** — GitHub 도구이고, JCR Q1
저널 사용 선례를 찾지 못했습니다(사용 사례는 arXiv 프리프린트와 RANLP 2025 학회 논문뿐이며 모두 문서
OCR 맥락, 장면 텍스트 사례는 없음). ② 앞서 "학습 코드가 없어 미세조정 불가"라고 적었던 것은 **사실과
다릅니다** — 공식 저장소에 `surya/scripts/finetune_ocr.py` 가 있습니다. 제외 사유를 틀린 근거로 쓸 수
없습니다. ③ 실제로 미세조정하려면 별도 환경이 필요합니다 — 설치된 `.venv_surya` 는 **surya-ocr 0.14.6 +
CPU 전용 torch** 라 학습이 불가능하고(해당 버전에는 finetune 스크립트도 없음), GPU torch 와 상위 버전을
새 환경에 깔아야 하는데 그러면 **제로샷으로 측정한 0.14 와 다른 모델**이 되어 비교 자체가 어긋납니다.
정리하면 Surya 를 넣으려면 "선례 없음"을 감수하거나 측정을 처음부터 다시 해야 하므로, 넣지 않는 쪽이
방어 가능합니다.
