...
> 🔎 **저장소 범위:** 본 저장소는 3모듈 전부를 다룹니다 — ①탐지 + ②OCR 구현·학습, **③의미 태깅은 로컬 VLM(gemma3:12b, Ollama)으로 구현·측정 완료**(D27, 4.13). 태깅 정답 데이터(`artifacts/gt/tagging_gt.csv`, 411크롭 검수)도 본 저장소에서 구축했습니다. 논문 본문의 태깅 수치(81.5/90.2/73.4)는 외부 GPT-4 결과로 본 측정과 프로토콜이 다릅니다.
...
| OCR | **하이브리드: OCR + VLM 교정·보강 (배포)** | GSV exact / WAR / CER | ✅ **exact 78.5% / WAR 0.741 / CER 0.139** (전역, 재라벨 GT — OCR 파이프라인(69.9%, D19~D24) 위에 gemma3:12b **후보 그라운딩 교정 + 전멸 라인 보강**(D28). OCR 전용 oracle 73.7%를 상회. 3.7·4.4·4.14) |
...
|--------|---------------|-----------|----------------------|------------------------|-----------------------|------------------------|
| Gangnam (Seoul) | Korean/English/Japanese | 100 | **0.853** | 79.7% / 0.182 | 0.098 | **79.4** (자체측정) |
| Brooklyn (New York) | English **(영어 전용 엔진·채점)** | 80 | **0.887** | **91.0% / 0.062** | **0.068** | **69.7** (자체측정) |
| Suwon (Ingye-dong) | Korean/Mixed Fonts | 70 | **0.899** | 63.5% / 0.263 | 0.189 | **78.0** (자체측정) |

> ⚠️ **위 표는 전부 "GT 크롭 기준", 즉 사람이 정확히 잘라준 간판에서의 성능 = 상한입니다.**
> 사진을 넣었을 때의 실제 성능(연쇄)은 **OCR 53.0% · 태깅 65.6%** 로 크게 낮습니다 (4.16).
> 논문에는 **두 표를 반드시 함께** 실으세요. 태깅 수치는 gemma3:12b + 42종 완전일치 +
> 기권 금지 조건이며, 논문 원본의 81.5/90.2/73.4는 GPT-4 기반이라 프로토콜이 다릅니다
> (4.15 ⑪).
...
| D29 | **RAG — 지역 상호 사전을 VLM 후보로 제공 (T5 재도입)** | 공개데이터로 사전 보강(소상공인 상가정보 + NYC 오픈데이터): 강남 5,660→**45,896** · 수원 667→**13,438**건, GT 상호 커버리지 강남 9.0→**21.5%** · 수원 1.2→**12.3%**. 태깅 tag exact 64.6→**69.4%**, **이득은 전부 in-DB(60.2→73.8%), off-DB는 66.1→67.8%로 사실상 불변** — 사전을 치환기가 아니라 후보로 두어 T5의 손상 모드가 재발하지 않음. 부수 발견 3건: ① 힌트 태그를 정답 어휘로 제약해야 함(브루클린 61.4→64.4%, 어휘밖 예측 20→11건) ② 코퍼스 확대가 순손실일 수 있음(뉴욕주 987건 추가 → 브루클린 −3.8%p, 기각) ③ **OCR 교정에는 무효과(+0.3%p, 변동폭 내) → run25 유지**. 동일설정 재실행 변동폭 1.2%p 실측 | 4.15 |
| D28 | **하이브리드 OCR 공식 채택 (run25) — VLM 교정·보강** | gemma3:12b로 ① 라인별 3모델 후보를 이미지 대조 확정(fixcand) ② VLM 단독 판독의 누락 라인 병합(compose) → 전역 **exact 69.9→78.5% / CER 0.163→0.139 / WAR 0.648→0.741** (강남 79.7 / 브루클린 **91.0** / 수원 63.5). VLM 단독은 61.7%로 OCR보다 나쁨 — **결합만이 둘 다를 상회**하고 OCR 전용 oracle(73.7)도 넘음. 1~2글자 오독 반감(49라인 회수). 비용: 크롭당 ~5s | 4.14 |
| D27 | **의미 태깅 구현 + VLM/OCR 경로 비교 (T6 완료)** | 태깅 GT 411크롭 검수 구축(`build_tagging_draft.py`+`review_tagging.py`, name 400/tag 395) → gemma3:12b(3모델 실측 선정)로 이미지 직접 vs OCR 텍스트 경로 비교. **tag: OCR 경로 64.6% > 이미지 56.7%** (한글권 +14~22%p — 도메인 미세조정 OCR의 몫), name: 이미지 48.2% > 텍스트 45.2%. 3모듈 파이프라인 구조의 정량 근거 확보. HF `dinov3-yolo11m-distilled-ocr` 검출기는 COCO 클래스 export로 판명·기각 | 4.13 |
...
| T5 | ~~OCR 사전(DB) 결합~~ | **종결 → D29로 부활** | 문자열 스냅 방식은 채택 안 함(손상이 이득을 상쇄). **사전을 치환기가 아니라 VLM 후보 제공자로 바꾼 RAG 형태로 재도입** — 태깅에서 in-DB +10.8%p, off-DB 무변화 (4.15) |
| T6 | ~~③ 의미 태깅 단계 구현~~ | **완료(D27)** | 로컬 VLM(gemma3:12b)으로 구현·측정. **업종 태깅은 OCR→LLM 경로가 이미지 직접보다 +7.9%p 우위**(64.6 vs 56.7%) — 3모듈 구조 정당화. 정답 데이터 `tagging_gt.csv`(411크롭 검수) 신규 구축 (4.13) |
...
간판 크롭
  ...
  │
  └─⑥ VLM 교정·보강 (gemma3:12b, Ollama):                              ← 4.14
         · fixcand: 라인별 3모델 후보 + 이미지 대조로 오독 글자 확정
         · compose: VLM 단독 판독에서 누락 라인 병합 (전멸 회수)
         → 최종 배포 출력 `ocr_{region}_{run}_ensemble.csv` (run25)
```
...
### 4.13 의미 태깅 — VLM(이미지) vs OCR→LLM 경로 비교 (T6 완료, D27)

파이프라인 3번째 모듈을 로컬 VLM으로 구현·측정했습니다. **핵심 질문은 "OCR 모듈이 여전히 제 몫을 하는가"** — 같은 모델에 ① 간판 크롭 이미지를 직접(vlm), ② 배포 OCR(run24) 출력 텍스트만(text) 주고 name/OSM 태그를 판별시켜 비교합니다 (`eval_vlm_tagging.py`, Ollama 로컬 실행 — transformers 핀과 무충돌).

**정답**: `tagging_gt.csv` (411크롭 전수 검수 — name 400 / tag 395건, 상호아님 10 제외. **업종 판별이 불가한 간판 5건은 tag 채점에서 제외** — 사람이 못 하는 판별을 모델에 요구하지 않음. 자유 입력 동의어는 채점 전 정규화). 태그 어휘 42종, 프롬프트에 "근거 없으면 unknown" 강제.

**모델 선정** (99크롭 예비 비교): gemma3:12b가 qwen2.5vl:7b·gemma3:4b를 전 지표에서 상회해 채택. 4B급끼리는 qwen이 우세했으나 체급 보정(12B) 후 역전 — 모델 선택 근거는 실측입니다.

**본 측정 (gemma3:12b, 전수):**

| 입력 경로 | name exact | name CER | **tag exact** | tag key | unknown |
|---|---|---|---|---|---|
| 이미지 직접 (vlm) | **48.2%** | **0.321** | 56.7% | 65.8% | 17.0% |
| **OCR 텍스트 (text)** | 45.2% | 0.420 | **64.6%** | **73.2%** | 16.7% |

**지역별 (name / tag exact):**

| | 강남 | 브루클린 | 수원 |
|---|---|---|---|
| vlm | 53% / 54% | **57% / 73%** | 34% / 43% |
| text | 44% / **68%** | 48% / 61% | **44% / 65%** |

**판독:**
- **업종 태깅은 OCR 경로가 +7.9%p 우위** (64.6 vs 56.7) — 탐지→OCR→LLM 3모듈 구조가 이미지 직접 입력보다 낫다는 정량 근거. **상호명 표기는 이미지 직접이 소폭 우위** (48.2 vs 45.2) — name은 글자 그대로 옮겨야 해 OCR의 1~2글자 오독이 그대로 전파되는 반면, tag는 그 오류에 강건하기 때문.
- **OCR 경로의 우위는 정확히 도메인 미세조정이 있는 곳에서 발생**: 한글권(강남 +14%p·수원 +22%p tag)은 간판 특화 v5 OCR이 VLM의 범용 시각 인코더가 못 읽는 것(캘리그래피·양각)을 읽어주고, 영어권 브루클린(zero-shot en OCR)은 vlm이 우위(73 vs 61). **모듈 협력의 가치가 미세조정 투자와 비례**한다는, 논문 서사에 정확히 부합하는 패턴.
- 환각 통제: "업종 불명" 검수 건에 태그를 단언한 사례 2건뿐, unknown 17%로 절제 확인.
- **주의(논문 명시)**: 논문 Table 3의 태깅 정확도(81.5/90.2/73.4)는 **외부 GPT-4** 결과로 프로토콜·정답이 달라 본 수치와 직접 비교 불가. 본 측정은 로컬 12B 오픈모델 + 42태그 exact 기준의 재현 가능한 프로토콜입니다.

> **후속(D29):** 이 OCR 텍스트 경로에 지역 상호 사전 검색을 붙이면 tag exact
> **64.6 → 68.6%** 가 됩니다. 다만 이득은 사전에 이미 있는 업소에서만 나오므로
> 층화해서 읽어야 합니다 — 4.15.

```bash
.venv/Scripts/python.exe eval_vlm_tagging.py --model gemma3:12b            # 전수, 양 경로
.venv/Scripts/python.exe build_tagging_draft.py && .venv/Scripts/python.exe review_tagging.py  # GT 재구축 시
```

### 4.14 하이브리드 OCR — VLM 교정·보강 (D28, 공식 채택 run25)

OCR 파이프라인(69.9%)의 잔여 오류를 분해하면 1~2글자 오독 16% + 완전 전멸 10%인데, 두 버킷을 VLM(gemma3:12b)이 각각 다른 방식으로 공략합니다 (`exp_vlm_ocr.py`):

- **fixcand (교정)**: 라인마다 v5·v4·zero-shot **3모델 후보**(`gen_line_candidates.py`)를 이미지와 함께 제시 → 사진 대조로 확정. 라인 수·순서 유지 강제, 응답 구조가 어긋나면 원문 롤백, "확신 없으면 첫 후보 유지" — 사전 스냅에서 배운 손상 방지 원칙의 적용.
- **compose (보강)**: fixcand는 라인 추가 금지라 전멸 버킷을 못 건드림 → VLM 단독 판독(solo)에서 fixcand 결과와 겹치지 않는 라인만 병합.

| 구성 | exact | CER | WAR | contain |
|---|---|---|---|---|
| OCR 파이프라인 (run24) | 69.9% | 0.163 | 0.648 | 75.2% |
| VLM 단독 (gemma3:12b) | 61.7% | 0.182 | 0.647 | 69.6% |
| + fix (배포 출력만 교정) | 76.2% | 0.144 | 0.715 | 83.6% |
| + fixcand (후보 그라운딩 교정) | 76.9% | 0.140 | 0.723 | 83.3% |
| **+ compose (전멸 보강) = 배포 run25** | **78.5%** | **0.139** | **0.741** | **84.8%** |

**지역별 (배포 run25):** 강남 79.7% / 0.182 — 브루클린 **91.0% / 0.062** — 수원 63.5% / 0.263.

**판독:**
- **완벽한 ablation 구조**: VLM 단독(61.7%)은 OCR(69.9%)보다 나쁘지만 결합(78.5%)은 둘 다를 크게 상회 — 어느 쪽도 다른 쪽을 대체하지 못하는 **진짜 시너지**. OCR이 후보를 대고 VLM이 이미지 대조로 심판하는 분업.
- **배포가 OCR 전용 oracle(73.7%)을 넘음** — VLM이 OCR 후보 집합 밖의 정보(시각 재판독)를 보탠다는 증거.
- 1~2글자 오독 버킷 16→8.8%로 반감(49라인 회수), 전멸 버킷은 compose가 회수.
- 비용: VLM 교정 크롭당 ~5s (Paddle 수십 ms 대비) — 오프라인 지도 구축 용도에는 수용 가능, 실시간에는 부적합함을 명시.
- 태깅(4.13)과 일관된 지역 패턴: VLM 기여는 영어권 최대(브루클린 +14.1%p), 어려운 한글 간판 최소(수원 +4.9%p).

```bash
.venv/Scripts/python.exe gen_line_candidates.py                       # 라인별 3모델 후보
.venv/Scripts/python.exe exp_vlm_ocr.py --mode fixcand                # VLM 교정
.venv/Scripts/python.exe exp_vlm_ocr.py --mode solo                   # VLM 단독(보강 재료)
.venv/Scripts/python.exe exp_vlm_ocr.py --mode compose --emit-run 25  # 최종 합성·공식 출력
```

### 4.15 RAG — 지역 상호 사전을 VLM에 후보로 제공 (D29)

T5(사전 스냅)를 **다른 구조로 되살린 실험**입니다. D14·D16에서 사전은 종결
처리됐는데, 실패 원인은 사전 자체가 아니라 **이미지 없이 문자열만으로 치환을
결정**한 것이었습니다 — SPRINT→SPRING(손상)과 istand→ISLAND(복구)가 둘 다
편집거리 1이라 어떤 규칙으로도 구분할 수 없었습니다. 하이브리드(4.14) 이후에는
**이미지를 보는 심판(VLM)이 이미 파이프라인 안에 있으므로**, 사전을 치환기가
아니라 후보 제공자로 강등시키면 그 판정이 문자열 문제가 아니라 시각 확인 문제가
됩니다. 이것이 이번 재도입의 유일한 근거입니다.

**① 사전 커버리지가 선행조건이었습니다.** 사전에 없는 상호는 RAG로 못 고치므로
커버리지가 곧 상한입니다. OSM만으로는 상한 자체가 없다시피 했습니다:

| 지역 | GT 라인 | POI 정확 존재 | 유사 검색 도달 | 사전 크기 |
|---|---|---|---|---|
| 강남 | 200 | 9.0% → **21.5%** | 35.5% → **56.0%** | 5,660 → 45,896 |
| 브루클린 | 199 | 17.6% → 17.6% | 64.8% → 65.3% | 3,600 |
| 수원 | 171 | **1.2% → 12.3%** | **9.9% → 49.1%** | 667 → 13,438 |

화살표 왼쪽이 OSM 전용, 오른쪽이 공개데이터 보강 후입니다 (한국: 소상공인시장진흥공단
상가(상권)정보 2026-06, 브루클린: NYC 영업허가 + 보건국 식당 점검). 전부 배포 시점에
접근 가능한 공개 데이터이며 평가 GT는 사전 구축에 일절 쓰지 않았습니다
(`artifacts/ocr_db/RAG_SETUP.md`). 특히 **수원은 OSM에 인계동 상권이 거의 등록돼
있지 않아** 검색이 원천적으로 불가능한 상태였습니다.

**② 누수 방어 — 반드시 층화 보고.** 논문의 기여가 "OSM 개선"인데 OSM을 입력으로
쓰면 순환 논리가 됩니다. 그래서 정답 상호가 사전에 있는 간판(in-DB, 기존 POI
검증)과 없는 간판(off-DB, 신규 POI 발굴)을 나눠 보고합니다. 층화에 쓰는 정답
상호명은 채점 시점에만 쓰며 모델 입력에는 들어가지 않습니다.

**③ 의미 태깅 결과 (400크롭 전수, gemma3:12b, `--mode rag`):**

| 입력 경로 | tag exact | tag key | 어휘밖 예측 | in-DB (n=102) | off-DB (n=293) | 강남 / 브루클린 / 수원 |
|---|---|---|---|---|---|---|
| 이미지 직접 (4.13) | 56.7% | 65.8% | — | 55.9% | 57.0% | 53.7 / **73.5** / 42.5 |
| **최종 = hybrid + 어휘제약 + 태그수정 (v6)** | **71.4%** | 78.7% | — | 70.6% | 71.7% | 75.7 / 65.2 / 73.2 |
| 위 + 기권 금지 (v7, 논문 비교용) | **75.7%** | 85.8% | — | 76.5% | 75.4% | 79.4 / 69.7 / 78.0 |
| OCR 텍스트 (4.13) | 64.6% | 73.2% | 21건 | 60.2% | 66.1% | 67.6 / 61.4 / 64.6 |
| OCR 텍스트 (동일설정 재실행) | 65.8% | 74.2% | — | 59.8% | 67.9% | 69.9 / 63.6 / 63.8 |
| RAG v1 — 검색 태그 그대로 | 68.6% | **79.0%** | 20건 | 69.9% | 68.2% | **75.7** / 61.4 / 68.5 |
| **RAG v2 — 힌트 태그를 정답 어휘로 제약 (채택)** | **69.4%** | 78.5% | **11건** | **73.8%** | 67.8% | 75.0 / **64.4** / **68.5** |
| RAG v3 — v2 + 뉴욕주 데이터 987건 (기각) | 68.1% | 77.0% | 13건 | 71.8% | 66.8% | 75.0 / 60.6 / 68.5 |

**판독:**
- **이득이 전부 in-DB에서 나옵니다 (59.8 → 70.6%, +10.8%p). off-DB는 67.9 →
  67.9%로 완전히 동일** — 사전에 없는 간판에서 이득도 손해도 없습니다. 이것이
  이 실험의 가장 중요한 결과입니다: 사전 45,896건 중 상당수가 그럴듯한 방해
  후보인데도(예: "강남빌딩" 질의에 "하남빌딩"이 검색됨) **T5에서 겪었던 손상이
  재발하지 않았습니다.** 사전을 치환기가 아니라 후보 제공자로 두고 이미지를 보는
  모델에게 기각권을 준 설계가 의도대로 작동한 것입니다.
- 따라서 논문 서술은 "RAG가 태깅을 개선한다"가 아니라 **"이미 등록된 POI를 다시
  확인하는 작업에서 개선하고, 신규 POI 발굴에서는 중립"** 이어야 정확합니다.
  순환 논리 지적은 이 분리로 방어됩니다.
- tag key 74.2 → 79.0%, unknown 16.2 → 13.9% — 검색된 후보가 업종 태그를 함께
  들고 오므로 판별이 추론에서 **조회**로 바뀌고, 그만큼 기권이 줄었습니다.
- 지역별로 **사전을 보강한 곳에서만** 오릅니다 (강남 +5.8%p, 수원 +4.7%p).

**④ 힌트는 정답 공간 안에서만 제안해야 합니다 (v1 → v2).** v1에서 브루클린만
61.4%로 정체했는데, 원인은 한글 오염(브루클린 코퍼스의 한글 항목은 **0건**)도
방해 후보도 아니라 **태그 체계 불일치**였습니다. 한국 POI 태그는 업종 매핑을
거쳐 구조적으로 42종 안에서만 나오는 반면, 브루클린은 OSM 원본 태그를 그대로
쓰는데 OSM 체계가 평가 어휘보다 세분화돼 있습니다 — 후보 태그의 **24.1%가 어휘
밖**이었습니다(강남 3.4%, 수원 0%). 그래서 검색은 성공하고 채점은 실패합니다:

| 간판 | 정답 | 검색 결과 | v1 예측 |
|---|---|---|---|
| StateFarm | office=company | State Farm (`office=insurance`) | ❌ office=insurance |
| DUTCH CLEANERS | shop=laundry | Sky/Yes/JSK Cleaners (`shop=dry_cleaning`) | ❌ shop=dry_cleaning |
| istand pops | amenity=cafe | ISLAND POPS (`amenity=ice_cream`) | ❌ amenity=ice_cream |

**업소를 정확히 찾아내고도 틀립니다.** 힌트에서 어휘 밖 태그만 떼고 상호명은
남기도록 고치자(v2) 브루클린 61.4 → **64.4%**, 어휘밖 예측 20 → **11건**,
in-DB 69.9 → **73.8%** 로 개선됐고 강남·수원은 불변(원래 어휘밖이 3.4%/0%)입니다.
전역 +0.8%p 자체는 변동폭 안이지만 세 지표가 같은 방향을 가리켜 인과는 분명합니다.
**외부 지식베이스를 좁은 라벨 체계에 붙일 때 일반적으로 발생하는 함정**입니다.

**⑤ 코퍼스를 키우는 것이 항상 이득은 아닙니다 (v3 기각).** 브루클린에서 사전에
없던 상호 88건의 업종 분포(슈퍼/식료 13 · 미용 13 · 이용 7 · 바 5)에 맞춰 뉴욕주
3종(식품점 383 · 미용/이용 225 · 주류 605)을 추가했습니다. 커버리지는 17.6 →
18.1%로 올랐지만 **태깅은 브루클린 64.4 → 60.6% 로 떨어졌고, 바뀐 예측 9건 중
개선 0 / 악화 5** 였습니다(강남·수원은 무변화 — 브루클린만 건드렸으므로 정합).
원인은 이름 방해가 아니라 **태그 신뢰도**였습니다:

- 태그 없이 이름만 넣은 항목(식품점 — 슈퍼/편의 경계가 모호해 일부러 비웠음)이
  힌트 목록의 **태그 신호를 희석**했습니다. `Rockaway GOURMET DELI & GRILL`은
  v2에서 convenience 태그 후보만 보여 정답이었는데, v3에서 무태그 항목이 끼어들자
  restaurant로 바뀌었습니다.
- 면허 종류로 유추한 태그가 경계 업종에서 오답을 유도했습니다 (미용 vs 이용:
  `Oachid Salon` beauty → hairdresser).

→ **기본 비활성으로 되돌렸습니다** (`build_poi_db.py --nys` 로만 켜짐). 남은
미수록 상호를 보면 `Nail Salon`·`PHARMACY`·`Dry Cleaning Service`처럼 상호명이
아니라 업종 표기를 GT가 이름으로 기록한 건이 많아, 브루클린의 잔여 격차는 데이터
부족이 아니라 **간판에 상호가 없는 경우**입니다. 어떤 등록부로도 못 메웁니다.

**⑨ 유의성 검정 — 무엇을 주장할 수 있는가 (필독).** %p 차이만으로 판단하면
안 됩니다. 대응표본 McNemar 검정과 부트스트랩 CI(5,000회)를 붙인 결과, **주장
가능한 것과 아닌 것이 갈립니다**:

| 비교 | n | 차이 | 95% CI | 뒤집힘 | p | |
|---|---|---|---|---|---|---|
| 텍스트 → RAG(최종) | 395 | **+6.8%p** | [+2.8, +10.9] | 47↑/20↓ | 0.001 | **유의** |
| └ in-DB 층 | 102 | +10.8%p | [+2.0, +19.6] | 16↑/5↓ | 0.027 | **유의** |
| └ off-DB 층 | 293 | +5.5%p | [+1.0, +9.9] | 31↑/15↓ | 0.026 | **유의** |
| 검색 품질(오프라인 hit@5): lexical → hybrid | 97 | +10.3%p | — | 10↑/**0**↓ | 0.002 | **유의** |
| 종단 태깅: lexical → hybrid | 395 | +2.0%p | [−1.0, +5.3] | 24↑/16↓ | 0.268 | **n.s.** |
| 기권 허용 → 기권 금지 | 395 | +4.3%p | — | 20↑/3↓ | 0.0005 | **유의** |

> **하이브리드 검색은 "검색을 개선한다"까지만 입증됐고 "최종 성능을 개선한다"는
> 미입증입니다(p=0.268).** 검색 품질 향상이 종단 성능으로 전달되지 않는 것 자체가
> 논의거리입니다 — 후보 5개 안에서 정답이 몇 번째인지는 VLM 판단에 별 영향이 없습니다.
> 마찬가지로 v3 데이터 추가의 −3.8%p, 태그 버그 수정 효과도 뒤집힌 예측이 한 자릿수라
> **잡음입니다**. 채택 여부는 유지하되(방향과 근거는 타당) 논문에 개선폭으로 인용하지 마세요.

**⑩ 기권 정책이 수치의 4.3%p를 좌우합니다.** 프롬프트가 "근거 없으면 unknown"을
강제해 기권율이 13.2%였습니다. 기권을 금지하면 **71.4 → 75.7%** (p=0.0005)로 오르지만,
**강제 선택된 52건 중 정답은 14건(27%)뿐이고 예측이 최빈 태그(restaurant 7·
office=company 5·convenience 5)로 몰립니다 — 능력 향상이 아니라 찍기입니다.**
기권 사례를 열어보면 `LI / Inc`, `WeLoue you656`, `RYERSON / 698 Nostrand Ave`처럼
**OCR이 상호를 못 읽어 전파된 실패**가 대부분이라 기권 자체는 정당했습니다.
논문에 다른 연구와 비교해 실을 때는 **상대 연구의 기권 허용 여부를 반드시 확인**하고
같은 조건의 값을 쓰세요.

> ⚠️ **변동폭 기준선 (실측):** 동일 프롬프트·동일 모델·temperature 0 재실행이
> tag exact **64.6% ↔ 65.8%** 로 갈렸습니다(Ollama의 배칭·KV 캐시 때문에 temp 0도
> 완전 결정적이지 않음). 사전 태그만 바꾼 v4↔v5는 **71.1% ↔ 70.9%(0.2%p)** 로
> 훨씬 촘촘했습니다 — 즉 최대 1.2%p까지 벌어질 수 있습니다. **1%p 안팎의 차이는
> 개선으로 주장하면 안 됩니다.** RAG의 in-DB +14.3%p(59.8→74.5)와 하이브리드의
> off-DB +3.7%p는 이 대역을 넘고, 검색 파라미터 튜닝(⑥ 각주)은 넘지 못합니다.

**⑥ 하이브리드 검색 (lexical + dense) — 채택.** 현행 검색기는 벡터가 아니라
문자 bigram 역색인 + 편집거리(lexical)입니다. 여기에 bge-m3 임베딩(dense)을
더해 RRF로 융합했습니다 (`rag_hybrid.py`).

*검색 품질만 따로 잰 오프라인 평가* — 질의는 배포 OCR이 읽은 라인(오독 포함),
정답은 그 크롭의 GT 상호가 사전에 실제로 존재하는 항목, 지표는 hit@k:

| 지역 | 검색기 | hit@1 | hit@3 | **hit@5** | hit@10 |
|---|---|---|---|---|---|
| 강남 (n=42) | lexical | 66.7% | 78.6% | 78.6% | 78.6% |
| | dense | 35.7% | 71.4% | 83.3% | 90.5% |
| | **hybrid** | **71.4%** | **83.3%** | **88.1%** | **95.2%** |
| 브루클린 (n=35) | lexical | **85.7%** | 88.6% | 91.4% | 94.3% |
| | dense | 37.1% | 68.6% | 77.1% | 82.9% |
| | **hybrid** | 77.1% | 88.6% | **94.3%** | **97.1%** |
| 수원 (n=20) | lexical | **55.0%** | 55.0% | 55.0% | 55.0% |
| | dense | 30.0% | 50.0% | 60.0% | 65.0% |
| | **hybrid** | 45.0% | **70.0%** | **75.0%** | **75.0%** |

**dense 단독은 어디서도 lexical을 못 이기는데(hit@1 30~37%) 융합하면 둘 다를
넘습니다** — 4.14의 하이브리드 OCR(VLM 단독 61.7% < OCR 69.9% < 결합 78.6%)과
같은 구조의 시너지입니다. 상보성의 근거는 실측 유사도에서 직접 보입니다:
`스타벅스↔Starbucks` dense 0.78 / lexical 0(문자체계가 달라 매칭 불가),
`스타벅스↔스타박스`(오타) dense 0.59 / lexical 0.75. **dense는 교차 문자체계·의미,
lexical은 오타에 강합니다.** 수원 lexical이 55%에서 완전히 평평했던 것(편집거리
필터에서 아예 걸러짐)이 75%까지 열린 것이 대표적입니다.

*종단 태깅 결과:*

| 구성 | tag exact | 어휘밖 | in-DB (n=102) | off-DB (n=293) | 강남/브루클린/수원 | 초/건 |
|---|---|---|---|---|---|---|
| v2 lexical | 69.4% | 11건 | **74.5%** | 67.6% | 75.0 / **64.4** / 68.5 | 3.9 |
| **v4 hybrid** | **71.1%** | **3건** | 70.6% | **71.3%** | **76.5** / 63.6 / **73.2** | 6.6 |

**검색기 선택이 총량보다 "어느 층이 좋아지는지"를 바꿉니다.** hybrid는 in-DB를
−3.9%p 희생하고 off-DB를 +3.7%p 올립니다. 오프라인 지표가 이를 예고했습니다 —
hybrid는 hit@5는 올리지만 **hit@1은 떨어뜨립니다**(dense가 상위를 흐림). 정답
업소가 사전에 있으면 lexical이 1순위로 꽂아주고, 없으면 dense가 같은 업종의 유사
업소를 끌어와 업종 추론을 돕습니다. **논문의 기여가 "OSM에 없는 간판 발굴"이므로
off-DB에 강한 hybrid가 목적에 부합**합니다. 비용은 질의 임베딩만큼(3.9→6.6초/크롭).

**⑦ 태그 매핑 오류가 항상 해롭지는 않습니다.** 상가정보 적재 규칙의 순서 버그로
**약국 453건 중 451건이 `shop=beauty`** 로 태깅돼 있었습니다(분류 문자열의
'화장품'이 '약국'보다 먼저 매칭). 고쳐서 재측정하니 400건 중 예측이 바뀐 것은
**1건**, 약국 정확도는 수정 전에도 14/15였습니다 — 간판에 "약국"이라 적혀 있으면
모델이 힌트를 볼 필요가 없기 때문입니다. ④의 브루클린 사례(StateFarm·CLEANERS)와
대조하면 규칙이 보입니다: **힌트 오류는 간판 텍스트만으로 업종이 결정되지 않을 때만
지배적입니다.** 한국어 간판은 업종어를 상호에 포함하는 관습(약국·의원·노래방)이
있어 힌트 의존도가 구조적으로 낮습니다. (데이터는 옳은 쪽으로 수정해 유지)

**⑧ OCR 교정에는 RAG가 무효과 — 기각.** 같은 사전을 하이브리드 OCR(4.14)의
후보 그라운딩 교정에 붙였습니다:

| 구성 | exact | CER | WAR | contain |
|---|---|---|---|---|
| fixcand (D28) | 76.9% | 0.140 | 0.723 | 83.3% |
| fixcand + RAG | 77.0% | 0.141 | 0.724 | 83.6% |
| compose = run25 (D28, **공식 유지**) | 78.6% | 0.139 | 0.741 | 84.8% |
| compose + RAG | 78.9% | 0.139 | 0.744 | 85.1% |

+0.3%p는 변동폭 안이라 채택하지 않습니다. **두 과제의 성격 차이로 설명됩니다** —
OCR 교정은 글자 단위 *시각 대조*라 3모델 후보에 이미 답이 있고 VLM이 사진을 보고
고르면 끝이라, 상호명 목록은 새로운 시각 증거가 되지 않습니다. 반면 태깅은 간판에
아예 적혀 있지 않은 정보(업종)를 추론하므로 외부 지식이 진짜 새 정보입니다.
**RAG는 "이미지에 없는 것을 채울 때" 값어치가 있고 "이미지에 있는 것을 읽을 때"는
무용합니다.**

**⑪ 논문 Table 3(GPT-4 기준)과의 위치.** 조건을 맞춘 값(기권 금지, v7)으로 비교하면
**격차는 전역 문제가 아니라 브루클린 하나입니다**:

| 지역 | 본 파이프라인 (기권 금지) | 논문 Table 3 (GPT-4) | 차이 |
|---|---|---|---|
| 수원 | **78.0%** | 73.4% | **+4.6%p** |
| 강남 | 79.4% | 81.5% | −2.1%p (변동폭 내) |
| 브루클린 | 69.7% | 90.2% | **−20.5%p** |

수원은 논문을 넘고 강남은 동등한데 브루클린만 벌어집니다. 원인은 4.13에서 이미
측정된 구조적 요인입니다 — **영어권에서는 OCR 경로(61.4%)보다 이미지 직접
경로(73.5%)가 우세**합니다. 브루클린 OCR은 zero-shot 영어 모델이라 도메인
미세조정 혜택이 없고, 텍스트만 넘기는 구조가 불리합니다. 즉 **"도메인 미세조정이
있는 언어권은 OCR 경로, 없는 언어권은 이미지 경로"** 라는 경로 분기가 측정에 근거한
설계 결정이 되며, 이를 검증하는 것이 `--mode vlmrag`(이미지 + POI 검색 후보)입니다.

**권장 구성:** 태깅 = `--mode rag --retriever hybrid` (힌트 태그는 정답 어휘로
제약, POI 레이어만 검색), OCR = run25 그대로(RAG 미적용). 다른 연구와 수치를
비교할 때는 `--no-abstain` 을 함께 쓰세요(⑩).

```bash
.venv/Scripts/python.exe build_poi_db.py --nyc --merge                     # 브루클린 자동 수집
.venv/Scripts/python.exe build_poi_db.py --kr-csv "<상가정보CSV>" --merge   # 한국 적재
.venv/Scripts/python.exe rag_retrieve.py --coverage                        # 커버리지=상한 확인
ollama pull bge-m3 && .venv/Scripts/python.exe rag_hybrid.py --build       # 임베딩(67k, 약 30분)
.venv/Scripts/python.exe rag_hybrid.py --eval                              # 검색기 hit@k 비교
.venv/Scripts/python.exe eval_vlm_tagging.py --model gemma3:12b --mode rag --retriever hybrid
.venv/Scripts/python.exe exp_vlm_ocr.py --mode fixcand --rag               # OCR 교정 RAG (기각)
```
...
**OCR 연쇄 결과 (라인 exact).** 배포 구성인 VLM 하이브리드 적용 전/후를 함께 싣습니다
(탐지 크롭에는 3모델 후보 파일이 없어 하이브리드는 `fixcand` 대신 `fix`를 씁니다 —
GT 크롭에서 76.9% vs 76.2%로 0.7%p 차이라 연쇄 추정에 미치는 영향은 작습니다):

| 지역 | GT 라인 | 연쇄 recall | TP 위 정확도 | GT 크롭 기준 |
|---|---|---|---|---|
| | | OCR만 → **+VLM** | OCR만 → **+VLM** | (상한) |
| 강남 | 212 | 50.0 → **56.1%** | 60.6 → **68.0%** | 63.7% |
| 브루클린 | 199 | 46.2 → **59.8%** | 54.8 → **70.8%** | 61.8% |
| 수원 | 181 | 32.0 → **42.0%** | 39.2 → **51.4%** | 46.4% |
| **전체** | **592** | **43.2 → 53.0%** | **52.1 → 64.0%** | **57.8%** |

> **하이브리드가 박스 품질 저하를 흡수합니다.** VLM 적용 전에는 TP 위 정확도(52.1%)가
> GT 크롭 기준(57.8%)보다 **낮았는데**, 적용 후에는 **높아집니다**(64.0% vs 57.8%).
> 탐지 박스가 GT보다 넉넉하면 OCR 검출기에는 불리하지만(옆 간판 혼입) VLM은 사진에서
> 맥락을 더 보므로 오히려 유리합니다. 즉 개선폭이 GT 크롭(+20.8%p)보다 탐지
> 크롭(+11.9%p)에서 다르게 나타나며, **파이프라인 견고성의 근거**가 됩니다.
>
> 다만 **연쇄 recall 53.0%가 시스템의 실제 성능**입니다. GT 크롭 기준 78.6%와의
> 차이 25.6%p는 전적으로 탐지 누락(FN 85개)에서 옵니다.
...
|---|---|---|---|---|---|---|---|
| 강남 | 136 | 109 | 27 | 69.1% | **86.2%** | 79.4% | 31/34 |
| 브루클린 | 132 | 115 | 17 | 62.9% | 72.2% | 69.7% | 44/49 |
| 수원 | 127 | 104 | 23 | 64.6% | 78.8% | 78.0% | 37/41 |
| **전체** | **395** | **328** | **67** | **65.6%** | **79.0%** | 75.7% | **112/124** |

- **허위 POI: FP 124건 중 112건(90%)에 업종 태그가 붙습니다.** 존재하지 않는 가게가
  상호명과 업종을 갖춘 채 지도에 등록될 수 있다는 뜻입니다. 지도 구축 응용에서
  **recall만이 아니라 precision이 결정적**이라는 정량 근거입니다.
- OCR과 마찬가지로 TP 위 정확도(79.0%)가 GT 크롭 기준(75.7%)보다 높습니다.
  단, **선택 효과를 명시해야 합니다** — 탐지가 성공한 간판은 애초에 크고 선명한
...
| 관점 | 탐지 | OCR | 태깅 |
|---|---|---|---|
| 모듈별 (GT 크롭 = **상한**) | mAP@0.5 0.880 | 78.6% | 75.7% |
| **연쇄 (실제 시스템)** | recall 0.816 | **53.0%** | **65.6%** |

*채점 단위가 달라 두 연쇄 수치를 직접 비교하면 안 됩니다 — OCR은 라인 단위
완전일치(엄격), 태깅은 크롭 단위 태그 일치입니다. 읽는 법: **간판을 찾아 업종까지
맞히는 비율 65.6%**, **간판 글자를 정확히 읽어내는 비율 53.0%**.*

> **다음 투자처는 탐지 recall입니다.** GT 크롭 78.6% ↔ 연쇄 53.0%의 25.6%p 차이는
> 전적으로 탐지 누락(FN 85개)에서 오며, 인식을 아무리 개선해도 이 손실은 줄지 않습니다.

> **논문 서술 권고:** 모듈별 표(탐지 0.88 / OCR 78.6% / 태깅 75.7%)와 연쇄 표를
...
| ⑥ | **VLM 후보 그라운딩 교정** (gemma3:12b fixcand) | **76.9%** (+7.0%p) | 0.140 | 4.14 |
| ⑦ | **VLM 전멸 라인 보강** (compose) | **78.5%** (+1.6%p) | 0.139 | 4.14 |
