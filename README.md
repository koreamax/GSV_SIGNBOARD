...
| D39 | **OCR 통제 비교 재정리 (앙상블 제외 · val/test 분리 · 신뢰구간)** | 4.19 표는 배포 성능용이라 앙상블 행이 섞이고 점추정뿐이었음. 단일 모델만 두고 사진 단위 val/test 분리 + 크롭 페어 부트스트랩 95% CI. **SVTRv2 > PP-OCRv5 단일 +5.2%p [+1.1, +9.5] (유의)**, PARSeq·CLOVA 는 PP-OCRv5 와 **구분 불가**(구간이 0 통과) → 4.19 의 'PARSeq 8%p 아래'는 과잉 주장이었음. 상용 API 는 SVTRv2 에만 유의하게 짐. 기하 파라미터가 같은 데이터에서 스윕된 점을 한계로 명시 | 4.21 |
...
**프로토콜.** 고정: 검출기(CRAFT∪DB) · 라인 병합(y_tol 0.04) · 패딩(0.04) · 전처리 · 채점 규칙.
바꾼 것은 **인식기 하나**. 미세조정군은 전부 같은 75,612 크롭 · 같은 signboard_v3 분할.
GSV 298장을 **사진 단위로 반씩** 갈라(seed 고정) test 절반에서만 차이를 보고합니다(같은 간판이 양쪽에 걸리지 않음).
신뢰구간은 **크롭 단위 페어 부트스트랩** 95%(2,000회).

| 인식기 | 학습 조건 | val exact | test exact | test CER |
|---|---|---|---|---|
| Tesseract 5.5 | 제로샷 | 28.7 | 32.9% | 0.468 |
| Surya 0.14 | 제로샷 | 39.6 | 41.2% | 0.332 |
| CLOVA OCR General | 제로샷(상용 API) | 53.8 | 62.6% | 0.194 |
| Tesseract 5.5 | 미세조정 | 31.4 | 34.9% | 0.457 |
| TrOCR-small | 미세조정 | 33.7 | 33.9% | 0.468 |
| EasyOCR (CRNN) | 미세조정 | 35.0 | 42.6% | 0.443 |
| PARSeq (ViT-S) | 미세조정 | 61.1 | 63.3% | 0.168 |
| PP-OCRv5 rec (단일 모델) | 미세조정 | 68.0 | 66.8% | 0.150 |
| **SVTRv2-B** | 미세조정 | 69.6 | **72.0%** | **0.131** |

**차이와 신뢰구간 (test 절반, %p):**

| 인식기 | vs PP-OCRv5 단일 | 95% CI | vs SVTRv2-B | 95% CI |
|---|---|---|---|---|
| CLOVA OCR General | −4.2 | [−10.2, +2.1] | −9.3 | [−14.4, −4.5] |
| PARSeq | −3.5 | [−8.3, +1.4] | −8.7 | [−13.0, −4.4] |
| PP-OCRv5 단일 | — | — | −5.2 | [−9.5, −1.1] |
| **SVTRv2-B** | **+5.2** | **[+1.1, +9.5]** | — | — |
| EasyOCR | −24.2 | [−30.1, −18.1] | −29.4 | [−35.2, −23.4] |
| Surya | −25.6 | [−31.4, −19.4] | −30.8 | [−36.1, −24.8] |
| Tesseract 미세조정 | −31.8 | [−37.2, −26.4] | −37.0 | [−42.7, −31.1] |
| TrOCR | −32.9 | [−39.6, −26.3] | −38.1 | [−44.1, −31.6] |
| Tesseract 제로샷 | −33.9 | [−39.2, −28.6] | −39.1 | [−45.1, −32.8] |

**살아남는 주장은 셋뿐입니다.**
1. **SVTRv2-B > PP-OCRv5 단일, +5.2%p [+1.1, +9.5].** 동일 데이터·동일 파이프라인이므로 차이의 원인은 아키텍처입니다.
2. **PARSeq 와 PP-OCRv5 는 구분되지 않습니다**(−3.5, 구간이 0을 지남). 4.19 에서 "PARSeq 가 8%p 아래"라고 쓴 것은 **과잉 주장**이었습니다.
3. **상용 API 도 PP-OCRv5 와는 구분되지 않고**(−4.2, 0을 지남), **SVTRv2 에는 집니다**(−9.3 [−14.4, −4.5]).
   따라서 "미세조정이 상용 API 를 이긴다"가 아니라 **"가장 좋은 미세조정 모델은 이기고, 평범한 것은 못 이긴다"** 가 정확한 서술입니다.

문서 OCR 계열(Tesseract·TrOCR·EasyOCR)은 24~39%p 차이에 구간이 0 근처에도 오지 않습니다. 같은 데이터로 미세조정해도
Tesseract 는 2.0%p 밖에 못 움직이므로, 이 격차는 데이터가 아니라 **구조**의 문제로 읽는 것이 맞습니다.

**한계.** 모든 행이 공유하는 기하 파라미터(pad 0.04, y_tol 0.04)는 과거에 **같은 GSV 데이터에서 스윕**해 정한 값입니다.
행 간 비교는 조건이 같아 공정하지만 **절대 수치는 모든 행에서 낙관적**입니다. 논문에 이 문장을 넣어야 합니다.
