...
step yolo26x artifacts/yolo26x_kfold_grouped/kfold_summary_map50.csv artifacts/yolo26x_kfold_grouped detection/train_yolo26x_kfold.py
step yolov5x artifacts/yolov5x_kfold_grouped/kfold_summary_map50.csv artifacts/yolov5x_kfold_grouped detection/train_yolov5x_kfold.py
step effdet  artifacts/effdet_kfold_grouped/kfold_summary_ap50.csv    artifacts/effdet_kfold_grouped  detection/train_effdet_kfold.py
step frcnn   artifacts/frcnn_kfold_v2_grouped/kfold_summary_ap50.csv  artifacts/frcnn_kfold_v2_grouped detection/train_frcnn_kfold.py

echo "[$(date '+%m-%d %H:%M')] 통합 프로토콜 재평가 시작" | tee -a "$STATUS"
DET_TAG=_grouped "$PY" detection/eval_det_unified.py --refresh \
...
"$PY" detection/eval_det_oof_ultra.py --kfold artifacts/yolo26x_kfold_grouped   --out artifacts/gt/det_oof_per_region_ultra_grouped.csv > "$LOG/eval_oof.log" 2>&1
